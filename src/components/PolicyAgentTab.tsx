...
export const PolicyAgentTab: React.FC = () => {
  ...
  const handleQuerySubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Find closest policy match or fallback
        ...
        <div className="flex flex-wrap gap-2">
          {POLICY_KNOWLEDGE_BASE.map((item) => (
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-lg bg-slate-900/90 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-800 hover:border-slate-700 transition flex items-center space-x-1.5 text-left"
            >
      ...
      <form onSubmit={handleQuerySubmit} className="relative">
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="w-full pl-12 pr-32 py-3.5 rounded-xl bg-slate-900/90 border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-white placeholder-slate-500 text-sm outline-none transition"
          />
          <button
            type="submit"
            disabled={isSynthesizing}
            className="absolute right-2 px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold transition flex items-center space-x-1.5 shadow-md shadow-indigo-600/30"
          >
            {isSynthesizing ? (
              <span>Reasoning...</span>
            ) : (
        ...
        <div className="lg:col-span-8 space-y-5">
          <div className="glass-panel p-6 rounded-2xl border border-indigo-500/20">
            ...
            <div className="flex flex-wrap items-center justify-between pb-4 border-b border-slate-800 gap-2">
              ...
              <span className="text-[10px] font-semibold uppercase px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                100% Policy Grounded
              </span>
            </div>
            ...
            {activePolicy.complianceWarning && (
              <div className="mt-5 p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 flex items-start space-x-3">
                <ShieldAlert className="h-5 w-5 text-rose-400 shrink-0 mt-0.5" />
                <div>
                  <span className="text-xs font-bold text-rose-300 block">Critical Compliance Guardrail</span>
            ...
            <div className="mt-5">
              ...
              <div className="space-y-2">
                {activePolicy.actionChecklist.map((act, idx) => (
                  <div key={idx} className="p-2.5 rounded-lg bg-slate-900/80 border border-slate-800 flex items-center space-x-2.5 text-xs text-slate-300">
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-800">
            ...
            <div className="space-y-2.5">
              {[
                ...
                <div key={i} className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 hover:border-slate-700 transition">
