    ...
    <div className="space-y-6 animate-fade-in text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="text-xs font-bold uppercase tracking-wider text-blue-200 px-1 flex items-center justify-between">
            <span>Organizational Capability Matrix</span>
            <span className="text-white">Target vs Current Baseline</span>
          </div>
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl cursor-pointer transition border ${
                    isSelected
                      ? 'bg-blue-800/80 border-blue-400 shadow-lg shadow-blue-500/20 ring-1 ring-blue-400/50'
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-900/90 text-blue-200 border border-blue-700">
                          {skill.category}
                        </span>
                      </div>
                      <div className="flex items-center space-x-3 mt-1.5 text-xs text-blue-200">
                        <span>Current: <strong className="text-white font-bold">{skill.currentMastery}%</strong></span>
                        <span>&bull;</span>
                        <span>Target: <strong className="text-white font-bold">{skill.orgDemandTarget}%</strong></span>
                        <span>&bull;</span>
                        <span className={`font-bold ${hasSevereGap ? 'text-amber-300' : 'text-emerald-300'}`}>
                          Gap: {skill.gap}%
                        </span>
                      </div>
                    </div>

                    <ChevronRight className={`h-5 w-5 transition ${isSelected ? 'text-blue-300 transform translate-x-1' : 'text-blue-500'}`} />
                  </div>
                  ...
                  <div className="mt-3.5 space-y-1">
                    ...
                    <div className="relative w-full bg-blue-950 rounded-full h-2.5 overflow-hidden border border-blue-900">
                      {/* Current Bar */}
                      <div
                      ...
                      {/* Target Marker */}
                      <div
                        className="absolute top-0 bottom-0 w-1.5 bg-amber-400 z-20 shadow-sm"
        ...
        <div className="lg:col-span-5 space-y-5">
          <div className="glass-panel p-6 rounded-2xl border border-blue-500/30 bg-blue-950/80">
            <div className="pb-4 border-b border-blue-800/80">
              <span className="text-[10px] font-mono uppercase font-bold px-2 py-0.5 rounded bg-blue-600/30 text-white border border-blue-400/40">
            ...
            {/* Gap Analysis Card */}
            <div className="mt-5 p-4 rounded-xl bg-blue-900/40 border border-blue-800">
              <div className="flex items-center justify-between text-xs mb-3">
                <span className="font-bold text-blue-200">Org Capability Health</span>
                <span className={`font-bold px-2 py-0.5 rounded-full ${
                  activeSkill.gap <= -30 ? 'bg-amber-500/30 text-amber-200 border border-amber-400/50' : 'bg-emerald-500/30 text-emerald-200'
                }`}>
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="p-2.5 rounded-lg bg-blue-950/80 border border-blue-800">
                  <span className="text-[10px] text-blue-200 block font-medium">Mastery</span>
                  <span className="text-lg font-bold text-white">{activeSkill.currentMastery}%</span>
                </div>
                <div className="p-2.5 rounded-lg bg-blue-950/80 border border-blue-800">
                  <span className="text-[10px] text-blue-200 block font-medium">Target</span>
                  <span className="text-lg font-bold text-blue-300">{activeSkill.orgDemandTarget}%</span>
                </div>
                <div className="p-2.5 rounded-lg bg-blue-950/80 border border-blue-800">
                  <span className="text-[10px] text-blue-200 block font-medium">Net Gap</span>
                  <span className="text-lg font-bold text-amber-300">{activeSkill.gap}%</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5">
              ...
              <div className="flex flex-wrap gap-2">
                {activeSkill.topEmployees.map((empName, i) => (
                  <span 
                    key={i}
                    className="px-3 py-1.5 rounded-lg bg-blue-900/50 border border-blue-700/60 text-xs text-white font-medium flex items-center space-x-1.5"
                  >
            ...
            <div className="mt-6 p-4 rounded-xl bg-gradient-to-br from-blue-900/60 via-indigo-900/30 to-blue-950 border border-blue-500/40">
              ...
              <div className="mt-4 pt-3 border-t border-blue-800/80 flex items-center justify-between">
                <span className="text-[11px] text-blue-200 font-normal">
                  {enrolledPrograms[activeSkill.id] ? 'Cohort initiated in LMS' : '8-week accelerated track'}
                </span>
                <button
